...
def _install_pat_observer(session: requests.Session) -> None:
    """Wrap the session so every response updates the shared PAT status.

    We patch at the ``send`` layer — this catches all requests going through
    the session (get, post, put, delete, patch) with a single hook and no
    per-call overhead beyond one attribute read.
    """
    original_send = session.send

    def observed_send(request, **kwargs):
        ...
        if code in (401, 403):
            ...
            try:
                ...
                pass
            pat_status.record_unauthorized(code, body)
        elif code and 200 <= code < 300:
            pat_status.record_success(code)
        return response

    session.send = observed_send  # type: ignore[assignment]
