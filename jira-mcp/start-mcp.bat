...
if exist requirements.txt (
    ...
    pip install fastapi-mcp fastapi httpx uvicorn jira python-dotenv pydantic --proxy http-proxy.porsche.org:3133
)
if %errorlevel% neq 0 (
...
if %MCP_PORT_FOUND%==0 if exist .env.example (
    ...
    echo Please copy .env.example to .env and add your JIRA credentials
    echo **********************************************************
    exit /b 1
)
...
for /f "delims=0123456789" %%c in ("%MCP_PORT%") do set MCP_PORT=8000
